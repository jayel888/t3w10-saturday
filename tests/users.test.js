...
describe("Users route", () => {
    ...
    test("'Get user by ID' route returns specific user only", async () => {
    ...
    test("'Login user' route returns specific user only", async () => {
        // POST localhost:3300/users/login
        const response = await request(app)
                        .post("/users/login")
                        // Ideally, we would set the authorisation header to be set by the value from JWT token
                        .set(
    ...
    test.skip("'Update/Edit user' route returns specific user only", async () => {
